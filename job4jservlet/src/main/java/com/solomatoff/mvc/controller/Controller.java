...
public class Controller {
    ...
    private void init() {
        if (actions.isEmpty()) {
            ...
            this.loadactionsuser(TypeAction.FINDBYIDROLE_USER, this.userFindByIdRole());
    ...
    public String getTypeUser(String login) {
        ...
        if (usersByLogin.size() > 0) {
            Integer idRole = usersByLogin.get(0).getIdRole();
            Role role = new Role(idRole, null, null);
            List<Role> rolesById = getLogic().findByIdRole(role);
            if (rolesById.size() > 0) {
                boolean isAdmin = rolesById.get(0).getIsAdmin();
                result = (isAdmin ? "admin" : "any user");
            }
        }
        return result;
    }
